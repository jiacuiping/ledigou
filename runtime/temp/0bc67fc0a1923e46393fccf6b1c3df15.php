<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:78:"D:\phpstudy_pro\WWW\ledigou\public/../application/admin\view\school\index.html";i:1575525364;}*/ ?>
<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
                        ...
                        <form class="layui-form" style="display: flex;float: left;width: 1000px;" id="myform" method="post">
                            ...
                            <div class="layui-form-item" style="margin-left: 15px">
                                <div class="layui-input-block">
                                    <input type="text" name="name" autocomplete="off" placeholder="学校名筛选" class="layui-input" value="<?php echo $condition['name']; ?>">
                                </div>
                            </div>

                            <div class="layui-form-item" style="margin-left: 15px;">
                                <div class="layui-input-block">
                                    <input type="text" name="campus" autocomplete="off" placeholder="校区筛选" class="layui-input" value="<?php echo $condition['campus']; ?>">
                                </div>
                            </div>
                    ...
                    <div class="layui-card-body">
                        ...
                        <script type="text/html" id="test-table-toolbar-toolbarDemo">
                            <div class="layui-btn-container">
                                <button class="layui-btn layui-btn-sm" id="create">创建学校</button>
                            </div>
                        </script>

                        <script type="text/html" id="test-table-switchTpl">
                            <input type="checkbox" name="school_status" lay-skin="switch" lay-text="正常|禁用" lay-filter="test-table-sexDemo"
                            value="{{ d.school_id }}" {{ d.school_status == 1 ? 'disabled' : '' }} data-json="{{ encodeURIComponent(JSON.stringify(d)) }}" {{ d.school_status == 1 ? 'checked' : '' }}>
                        </script>
    ...
    <script>
        ...
        layui.config({
            ...
            table.render({
                ...
                ,cols: [[
                        {type: 'checkbox'}
                        ,{field:'school_id', title:'ID', width:100, unresize: true, sort: true}
                        ,{field:'school_name', title:'学校名称'}
                        ,{field:'school_campus', title:'校区名称'}
                        ,{field:'school_address', title:'详细地址'}
                        ,{field:'school_status', title:'学校状态', width:120, templet: '#test-table-switchTpl', unresize: true}
                        ,{field:'school_time', title:'创建时间', minWidth:80, sort: true}
            ...
            table.on('tool(test-table-form)', function(obj){
                ...
                if(obj.event === 'update'){
                    ...
                    layer.confirm('您确定要删除该学校吗？', function(index){
                        $.ajax({
                            url:url,
                            success:function(res){
                                layer.msg(res.msg);
