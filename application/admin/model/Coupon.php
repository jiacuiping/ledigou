...
class Coupon extends Model
{
    ...
    protected $createTime = 'coupon_time';
    protected $rule = [
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='coupon_id desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='coupon_id desc')
    {
    ...
    public function UpdateData($param)
    {
        ...
        $res = $this->allowField(true)->save($param, ['coupon_id' => $param['coupon_id']]);
    ...
    {
        return $this->where('coupon_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
}