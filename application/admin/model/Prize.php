...
class Prize extends Model
{
    ...
    protected $createTime = 'prize_time';
    protected $rule = [
        'prize_name|奖品名称'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='prize_id desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='prize_id desc')
    {
    ...
    {
        return $this->where('prize_id',$id)->find();
    }
    ...
    public function UpdateData($param)
    {
        ...
        $res = $this->allowField(true)->save($param, ['prize_id' => $param['prize_id']]);
    ...
    {
        return $this->where('prize_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
     ...
     * @param array $where 条件
     **/
    public function GetPrizeSum($where = [])
    {
        $where['prize_status'] = 1;
        return $this->where($where)->sum('prize_probability');
    }
}