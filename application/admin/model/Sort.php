...
class Sort extends Model
{
    ...
    protected $table = 'xin_goods_sort';
    protected $rule = [
        'sort_name|分类名称'    => 'require|unique:goods_sort',
        'sort_icon|分类图标'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='sort_rank desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='sort_rank desc')
    {
    ...
    {
        return $this->where('sort_id',$id)->find();
    }
    ...
    public function UpdateData($param)
    {
        ...
        $res = $this->allowField(true)->save($param, ['sort_id' => $param['sort_id']]);
    ...
    {
        return $this->where('sort_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
}