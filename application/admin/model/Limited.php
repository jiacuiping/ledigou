...
class Limited extends Model
{
    ...
    protected $createTime = 'limited_time';
    protected $rule = [
        'limited_goods|秒杀商品'    => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='limited_time desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='limited_time desc')
    {
    ...
    {
        return $this->where('limited_id',$id)->find();
    }
    ...
    public function UpdateData($param)
    {
        ...
        $res = $this->allowField(true)->save($param, ['limited_id' => $param['limited_id']]);
    ...
    {
        return $this->where('limited_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
}