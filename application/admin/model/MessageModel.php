...
class MessageModel extends Model
{
    ...
    protected $createTime = 'model_time';
    protected $table = 'xin_message_model';
    protected $rule = [
        'model_type|模版类型'       => 'require',
        'model_title|模版标题'      => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='model_id desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='model_id desc')
    {
    ...
    {
        return $this->where('model_id',$id)->find();
    }
    ...
    {
        $res = $this->allowField(true)->save($param, ['model_id' => $param['model_id']]);
    ...
    {
        return $this->where('model_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
}