...
class Address extends Model
{
    ...
    protected $createTime = 'address_time';
    protected $rule = [
        'address_user|用户信息'       => 'require',
    ];
     ...
     **/
    public function GetListByPage($where=array(), $page=1, $limit=10, $order='address_id desc')
    {   
        return $this->where($where)->page($page,$limit)->order($order)->select();
    }

    //获取数据列表，不分页
    public function GetDataList($where=array(), $order='address_id desc')
    {
    ...
    public function UpdateData($param)
    {
        ...
        $res = $this->allowField(true)->save($param, ['address_id' => $param['address_id']]);
    ...
    {
        return $this->where('address_id',$id)->delete() ? array('code'=>1,'msg'=>'删除成功') : array('code'=>0,'msg'=>'删除失败');
    }
}